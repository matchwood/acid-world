...
class AcidWorldBackend (b :: k) where
  ...
  handleUpdateEventC :: (All (IsValidEvent ss nn) (firstN ': ns), All (ValidEventName ss) (firstN ': ns), MonadIO m, ValidAcidWorldState u ss) => (NP (StorableEvent ss nn) (firstN ': ns) -> AWBSerialiseT b) ->  (AWBState b) -> (AWState u ss) -> EventC (firstN ': ns) -> (EventResult firstN -> m ioRes) -> m (Either AWException  (EventResult firstN, ioRes))
